...
// Four benches duplicate binders over fresh algebras, so their
// preludes spell the locked trio (Sigma/Copy/Copiable) and a proved
// .copy witness per shared algebra, each congruence a one-rewrite
// local lemma -- the Cop recipe; generics needs none. Five benches,
// each sized by a module count n:
//
...
// gen_cop: the locked trio spelled as base spells it (lock_valid
// compares by conversion), plus the one- and two-argument congruence
// lemmas the .copy witnesses below rebuild their proofs with
function gen_cop(): string {
  let s = "type Sigma<-A: Type, -B: @-x: A -> Type>:\n"
    + "  Tuple{fst: A, snd: B(fst)}\n\n";
  s += gen_def("Pair", ["-A: Type", "-B: Type"], "Type", ["A", "B"],
    "  Sigma<A, _ => B>");
  s += gen_def("Copy", ["-A: Type", "-x: A"], "Type", ["A", "x"],
    "  &y: A -> {x == y : A}");
  s += gen_def("Copiable", ["-T: Type"], "Type", ["T"],
    "  @x: T -> Copy(T, x) & Copy(T, x)");
  s += gen_def("Equal.cong2",
    ["-A: Type", "-B: Type", "-C: Type", "-f: A -> @y: B -> C",
      "-a: A", "-c: A", "-b: B", "-d: B", "ea: {a == c : A}",
      "eb: {b == d : B}"],
    "{f(a)(b) == f(c)(d) : C}",
    ["A", "B", "C", "f", "a", "c", "b", "d", "ea", "eb"],
    "  %eb : {f(a)(b) == f(c)(_) : C}\n"
    + "  %ea : {f(a)(b) == f(_)(b) : C}\n  {==}");
  return s;
}

// gen_copy: the proved copy witness of a nat-shaped algebra; each
// component's proof is an inline one-rewrite congruence
function gen_copy(T: string, Z: string, S: string): string {
  const z = Z + "{}";
  const sc = (e: string): string =>
    "%" + e + " : {" + S + "{p} == " + S + "{_} : " + T + "}; {==}";
  return gen_def(T + ".copy", ["n: " + T],
    "Copy(" + T + ", n) & Copy(" + T + ", n)", ["n"],
    "  match n:\n    case " + z + ":\n      ((" + z + ", {==}), ("
    + z + ", {==}))\n    case " + S + "{p}:\n"
    + "      ((a, ea), (b, eb)) = " + T + ".copy(p)\n"
...
// gen_copy_tree: the proved copy witness of a leaf/node tree whose
// leaf holds a Nat; leaf proofs rewrite inline, node proofs ride
// Equal.cong2
function gen_copy_tree(T: string, L: string, N: string): string {
  const lc = (e: string): string =>
    "%" + e + " : {" + L + "{v} == " + L + "{_} : " + T + "}; {==}";
  const nc = (i: string): string =>
    "Equal.cong2(" + T + ", " + T + ", " + T + ", x => y => " + N
    + "{x, y}, l, l" + i + ", r, r" + i + ", el" + i + ", er" + i + ")";
  return gen_def(T + ".copy", ["t: " + T],
    "Copy(" + T + ", t) & Copy(" + T + ", t)", ["t"],
    "  match t:\n    case " + L + "{v}:\n"
    + "      ((v1, e1), (v2, e2)) = Nat.copy(v)\n"
    + "      ((" + L + "{v1}, " + lc("e1") + "), ("
    + L + "{v2}, " + lc("e2") + "))\n"
    + "    case " + N + "{l, r}:\n"
    + "      ((l1, el1), (l2, el2)) = " + T + ".copy(l)\n"
    + "      ((r1, er1), (r2, er2)) = " + T + ".copy(r)\n"
    + "      ((" + N + "{l1, r1}, " + nc("1") + "), ("
    + N + "{l2, r2}, " + nc("2") + "))");
}
