...
- Bend's core is [formalized in Lean](bend2/bend.lean): the checker's rules, with kinds, `Data` copying and natives, are proven confluent, sound, normalizing and consistent with `Type : Type` and negative datatypes. Read the paper: [BendTT: An Affine Dependent Type Theory](docs/BendTT.pdf).
