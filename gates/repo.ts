...
allow(/^demos\/[a-z0-9_]+\/[A-Za-z_]+\.bend$/, 30000);
allow(/^demos\/[a-z0-9_]+\/[A-Za-z_]+\.(c|sh|md)$/, 4000);
allow(/^demos\/[a-z_]+\/web\/(index\.html|main\.js|bunfig\.toml)$/, 4000);
allow("docs/GUIDE.md", 12000);
allow(/^docs\/(BendRT|BendTT)\.pdf$/, 400000, true);
allow(/^docs\/intro\.(gif|mp4)$/, 25000000, true);
allow(/^docs\/assets\/[a-z_]+\.svg$/, 40000, true);
