...
answers the command line less the runtime's own options (`bend`'s, or a binary's
`--threads` and `--gpu`; a `--` ends them). A handle
...
bend file.bend a b        # the same; IO.args answers ["a", "b"]
