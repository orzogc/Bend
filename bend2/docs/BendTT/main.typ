...
the surface as a quantity on kinds: a datatype declares whether its
...
kernel and into the surface, as a quantity each datatype declares
and the checker verifies against its constructors
(@sec:grades).

The implemented language is larger than the kernel: machine words
and floats, flat arrays, and the surface kind system itself
...
#co[Data] under the kind system of @sec:grades. One program hint,
...
#co[+x] belongs to the kind system of @sec:grades, not to the
kernel.
...
surface kind system of @sec:grades, which the kernel never sees
as a rule.
...
lemma at once. Bend re-admits it through a _quantity on every kind_.
The one sort of the kernel splits: a type's kind is #Kd($q$) over
a quantity $q$, the same quantities binders take: #g1 (one use,
the bare binder) or #g2 (many, the #co[+] binder), with
#Ty $=$ #Kd(g1) and #Da $=$ #Kd(g2). A binder of quantity $q$
needs a type whose kind fits #Kd($q$); so a binder of quantity
$omega$ (surface #co[+x], or the local #co[+x = v]) is permitted
if and only if its type's kind reduces to #Da. This is the only
duplication mechanism in the language; nothing else contracts.

#figure(caption: [The kind extension. The rules of @fig:typing
stay; these are added over them. $a inter.sq b$ is the quantity
...
stuck quantities is stuck. The last two rules are the subsumptions:
...
quantity may depend on the parameters. At the conversion rule every
...
types, and generic code is _quantity-polymorphic_, taking the quantity
as an erased parameter (#co[forall -a: Quant]). A $omega$ binder
needs a kind whose quantity _reduces_ to #g2, so a parameter quantity
licenses nothing: a polymorphic function cannot duplicate its
argument, and #co[+] becomes available only after instantiation.

_Declarations earn their kind._ A datatype is declared
#co[type D\<p_1, .., p_n\> is Kind(G):] with $G$ a quantity over the
...
_live_ field's quantity $H$ be _entailed_ by $G$, written
$G models H$. Entailment is over atoms:
$"atoms"(#g2) = nothing$, $"atoms"(#g1) = {#g1}$,
$"atoms"(a inter.sq b) = "atoms"(a) union "atoms"(b)$, a stuck quantity
is its own atom, and $G models H$ holds when
$#g1 in "atoms"(G)$ or $"atoms"(H) subset.eq "atoms"(G)$ up to
conversion. So #g2 entails only quantities that reduce to #g2, #g1
entails everything, and a parameter quantity entails itself. An
...
parameter quantity contains, so a closure can sit only in a family
...
kind is #Kd($a inter.sq #g2$) $=$ #Kd($a$), the quantity of $T$ itself.
...
The kind system is an implementation extension, not a kernel
...
elaboration is planned work. Quantities are erased at runtime, like
...
the kind system of @sec:grades, whose kernel-side meaning is the
...
consistency argument, and formalizing the kind system is the
...
captures, and no declaration can declare it reusable. There is no
...
the copying permit is a surface quantity that each datatype
...
against the declared quantity, in the constructor's own context, and
a function type is #Ty there, as is any constructor-local type or
quantity (@sec:grades). And the kind system lives outside the
...
for pairs. The kind system is argued and audited, but not yet
...
back with quantities, declared per datatype and verified at every
...
the kind system first, and perhaps toward stronger equalities:
